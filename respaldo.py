...
EXIT_SECONDS_AFTER_ENTRY = 10   # segundos que tarda en tomar la salida después de haber tomado la entrada
...
#DIST_FALLBACK_THRESHOLD = 0.60 # umbral distancia fallback
...
#                        EXTRAER Y PREPARAR EL VECTOR DEL ROSTRO
                        ...
                        if timer:
                            ...
                            # Determinar mensaje
                            if remaining > 0:
                                tipo = "Salida" if timer["state"] == "entrada" else "Entrada"
                                txt = f"{tipo} en: {remaining}s"
                            else:
                                tipo = "salida" if timer["state"] == "entrada" else "entrada"
                                txt = f"Listo para {tipo}"

                            # ---- Mostrar texto centrado arriba ----
                            h, w = display.shape[:2]

                            # Medir texto una sola vez
                            (tw, th), baseline = cv2.getTextSize(txt, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
                            x = (w - tw) // 2
                            y = th + 20  # pequeña separación superior
...
def admin_mostrar_usuarios():
    ...
    for nombre, user in db.items():
        print(
            f"- {nombre}\n"
            f"    Registro: {user.get('registro', '-')}\n"
            f"    Grupo:    {user.get('group', '-')}\n"
            f"    Materia:  {user.get('subject', '-')}\n"
        )
...
def main():
    ...
    while True:
        ...
        elif opcion == "4": # Panel de administración
            admin_menu()
        elif opcion == "5":
